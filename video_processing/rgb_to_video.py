...
examples = [
    os.path.join(frames_folder_rgb, p) for p in os.listdir(frames_folder_rgb)
]
for example in examples:
    ...
    methods = [os.path.join(example, p) for p in os.listdir(example)]
    for method in methods:
        method_name = os.path.split(method)[1]
        video_method = os.path.join(video_example, method_name)
        os.mkdir(video_method)

        trials = [os.path.join(method, p) for p in os.listdir(method)]

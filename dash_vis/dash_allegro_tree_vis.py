...
q_u_nodes = q_nodes[:, q_dynamics.get_q_u_indices_into_x()[4:]]
...
for i_node in tree.nodes:
    ...
    x_edges += [q_u_nodes[i_node, 0], q_u_nodes[i_parent, 0], None]
    y_edges += [q_u_nodes[i_node, 1], q_u_nodes[i_parent, 1], None]
    z_edges += [q_u_nodes[i_node, 2], q_u_nodes[i_parent, 2], None]
...
def create_tree_plot_up_to_node(num_nodes: int):
    ...
    goal_plot = make_large_point_3d(np.array([-0.03, -0.043, 0.045]), name='goal', color="green")
...
fig = go.Figure(data=create_tree_plot_up_to_node(n_nodes),
                layout=layout)
fig.update_layout(scene=dict(xaxis_title_text='x',
                             yaxis_title_text='y',
                             zaxis_title_text='z',))

...
class IrsMpcQuasistatic:
    ...
    def get_q_sim_params(self, p_mpc: IrsMpcQuasistaticParameters):
        ...
        p.h = p_mpc.h
        p.forward_mode = kSmoothingMode2ForwardDynamicsModeMap[p_mpc.smoothing_mode]
    ...
    def initialize_problem(
        ...
        self.x_trj_0 = self.rollout(x0, u_trj_0, self.sim_params_rollout.forward_mode)
        self.u_trj_0 = u_trj_0  # T x m
        self.A_trj, self.B_trj, self.c_trj = self.calc_bundled_ABc_trj(
            self.x_trj_0[: self.T], self.u_trj_0
        )

    def get_Q_mat_from_Q_dict(self, Q_dict: Dict[ModelInstanceIndex, np.ndarray]):
        Q = np.eye(self.dim_x)
        for model, idx in self.q_sim.get_position_indices().items():
            Q[idx, idx] = Q_dict[model]
        return Q

    def get_R_mat_from_R_dict(self, R_dict: Dict[ModelInstanceIndex, np.ndarray]):
        R = np.eye(self.dim_u)
        i_start = 0
    ...
    def calc_bundled_ABc_trj(self, x_trj: np.ndarray, u_trj: np.ndarray):
        ...
        if self.irs_mpc_params.smoothing_mode in k1RandomizedSmoothingModes:
            ...
            (A_trj, B_trj, x_next_smooth_trj,) = self.q_sim_batch.calc_bundled_ABc_trj(
                x_trj,
                u_trj,
                std_u,
                sim_p,
            ...
            sim_p.log_barrier_weight = self.irs_mpc_params.calc_log_barrier_weight(
        ...
        for t in range(T):
            ...
            c_trj[t] = x_next_nominal - A_trj[t].dot(x_trj[t]) - B_trj[t].dot(u_trj[t])
    ...
    def calc_bundled_ABc(self, x_nominal: np.ndarray, u_nominal: np.ndarray):
        ...
        if self.irs_mpc_params.smoothing_mode in k1RandomizedSmoothingModes:
            ...
            x_batch[:] = x_nominal
            u_batch = np.random.normal(u_nominal, std_u, (n_samples, self.dim_u))
            (
                x_next_batch,
                A_batch,
                B_batch,
            ...
            x_next_smooth = self.q_sim.calc_dynamics(x_nominal, u_nominal, sim_p)
            if self.irs_mpc_params.use_A:

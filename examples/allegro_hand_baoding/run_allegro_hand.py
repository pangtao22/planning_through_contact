...
q_a0 = np.array([0.03501504, 0.75276565, 0.74146232, 0.83261002, 0.63256269,
                 ...
                 0.90415436])
q_u0_r = np.array([1, 0, 0, 0, -0.03, 0.0225, 0.048])
q_u0_g = np.array([1, 0, 0, 0, -0.05, -0.0475, 0.048])
# q_u0_r = np.array([1, 0, 0, 0, -0.015, 0.0, 0.031])
# q_u0_g = np.array([1, 0, 0, 0, -0.08, -0.0, 0.031])
...
params.Q_dict = {
    idx_u_r: np.array([10, 10, 10, 10, 1, 1, 1]),
    idx_u_g: np.array([10, 10, 10, 10, 1, 1, 1]),    
...
#%%
Q_WB_d = RollPitchYaw(0, 0, np.pi + 0.8).ToQuaternion()
p_WB_d = q_u0_r[4:] + np.array([-0.2, 0.2, 0.2], dtype=float)
q_d_dict = {idx_u_r: np.hstack([Q_WB_d.wxyz(), p_WB_d]),
            idx_u_g: np.hstack([Q_WB_d.wxyz(), p_WB_d]),
            idx_a: q_a0}
...
u_trj_0[:,2] = np.linspace(-0.3, 0.1, T)

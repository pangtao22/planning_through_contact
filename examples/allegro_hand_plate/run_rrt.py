...
contact_sampler = AllegroHandPlateContactSampler(q_sim, q_sim_py)
...
rrt_params.goal[q_sim.get_q_u_indices_into_q()[5]] = -0.3
rrt_params.goal[q_sim.get_q_u_indices_into_q()[6]] = 0.3
...
q_vis.draw_object_triad(length=0.1, radius=0.001, opacity=1, path="plate/box")
q_vis.draw_goal_triad(
    length=0.4,
    radius=0.01,
    opacity=0.7,
    X_WG=RigidTransform(Q_WB_d, np.array([0.0, -0.3, 0.3])),
)
prob_rrt.iterate()

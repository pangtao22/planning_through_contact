...
from planar_hand_plotly_vis import (pink, dark_red, bright_blue, blue,
...
q_dict1 = {model_u: np.array([0, 0.3, 0]),
...
def create_ellipsoid_plot(x_bar, u_bar, **kwargs):
    Bhat, chat = reachable_set.calc_bundled_Bc(q=x_bar, ubar=u_bar)
    ...
    return go.Mesh3d(
        dict(x=e_points[0], y=e_points[1], z=e_points[2], alphahull=0,
             opacity=0.3, name='ellipsoid')), c_u_hat, cov_u


def create_pca_plots(cov_u: np.ndarray, name: str, color: str):
    ...
    ppW_z = []
    for i in range(3):
        ppW_x += [principal_points_W[i, 0, 0], principal_points_W[i, 1, 0],
                  None]
        ppW_y += [principal_points_W[i, 0, 1], principal_points_W[i, 1, 1],
                  None]
        ppW_z += [principal_points_W[i, 0, 2], principal_points_W[i, 1, 2],
                  None]
...
plots += create_major_minor_plots(0, p_W_major0, p_W_minor0,
                                  colors=colors0)
plots += create_major_minor_plots(1, p_W_major1, p_W_minor1,
                                  colors=colors1)
...
#%%
plot_robots_with_major_and_minor_goals(x=x0, qu_goal_major=qu0_goal_major,
                                       qu_goal_minor=qu0_goal_minor,
                                       colors=colors0)


#%%
plot_robots_with_major_and_minor_goals(x=x1, qu_goal_major=qu1_goal_major,

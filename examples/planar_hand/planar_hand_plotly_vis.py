...
def draw_ball(y: float, z: float, theta: float, fill_color: str,
              line_color: str, line_dash: Union[str, None], fig):
    fig.add_shape(
        type="circle",
        x0=y - 0.25,
        x1=y + 0.25,
    ...
    fig.add_shape(type='line', x0=y, y0=z,
...
def plot_planar_hand_from_vector(x, fig,
                                 color_ball_fill, color_ball_line,
                                 color_arms_fill, color_arms_line):
    def make_R(theta):
        R = np.array([
    ...
    draw_ball(y=y_ball, z=z_ball, theta=theta_ball,
              fill_color=color_ball_fill, line_color=color_ball_line,
              line_dash=None, fig=fig)

    body_1 = make_R(q1_l).dot(polygon1)[0:2] + np.array([circle_l0]).transpose()
    fig.add_trace(
        go.Scatter(x=body_1[0, :], y=body_1[1, :], fill='toself',
                   fillcolor=color_arms_fill, line_color=color_arms_line,
                   line_width=line_width))

    body_2 = make_R(q1_l + q2_l).dot(polygon2)[0:2] + np.array(
        [circle_l1]).transpose()
    fig.add_trace(
        go.Scatter(x=body_2[0, :], y=body_2[1, :], fill='toself',
                   fillcolor=color_arms_fill, line_color=color_arms_line,
                   line_width=line_width))

    body_3 = make_R(q1_r).dot(polygon1)[0:2] + np.array([circle_r0]).transpose()
    fig.add_trace(
        go.Scatter(x=body_3[0, :], y=body_3[1, :], fill='toself',
                   fillcolor=color_arms_fill, line_color=color_arms_line,
                   line_width=line_width))

    body_4 = make_R(q1_r + q2_r).dot(polygon2)[0:2] + np.array(
        [circle_r1]).transpose()
    fig.add_trace(
        go.Scatter(x=body_4[0, :], y=body_4[1, :], fill='toself',
                   fillcolor=color_arms_fill, line_color=color_arms_line,
                   line_width=line_width))


def plot_robots_with_major_and_minor_goals(x, qu_goal_major, qu_goal_minor,
                                           colors):
    fig = go.Figure()

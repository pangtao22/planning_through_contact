...
app.layout = dbc.Container(
    [
        dbc.Row(
            [
                dbc.Col(
                    dcc.Graph(id="reachable-sets", figure={}),
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.Div(id="q_u0_display", children="Current q_u0:"),
                        html.H3("2. sample grasp"),
                        html.Button(
                            "Sample Grasp", id="sample_grasp", n_clicks=0
                        ),
                ...
                dbc.Col(
                    [
                        ...
                        html.Button(
                            "Compute and Draw",
                            id="btn-1-step-reachability",
                            n_clicks=0,
                        ),
                        html.H3("4. Calc Trajectory"),
                        dcc.Dropdown(
                            ["Randomized Smoothing", "Analytic Smoothing"],
                            "Analytic Smoothing",
                            id="smoothing-scheme",
                        ),
...
def update_qa0(n_clicks, q_u0_json):
    if q_u0_json is None:
        return json.dumps({"qa_l": [0.0, 0.0], "qa_r": [0.0, 0.0]}), html.Div(
            ""
        )
...
def update_reachability(n_clicks, q_u0_json, q_a0_json):
    ...
    return fig, json.dumps(
        {"qa_l": qa_l_samples.tolist(), "qa_r": qa_r_samples.tolist()}
    )
...
def click_callback(click_data, figure):
    ...
    return msg, json.dumps(
        {"y": point["x"], "z": point["y"], "theta": point["z"]}
    )
...
@app.callback(
    ...
    [
        State("selected-goal", "data"),
        State("q_u0", "data"),
        State("q_a0", "data"),
        State("smoothing-scheme", "value"),
    ],
)
def calc_trajectory(
    n_clicks, q_u_goal_json, q_u0_json, q_a0_json, smoothing_scheme
):
    ...
    irs_mpc.irs_mpc_params.bundle_mode = smoothing_scheme_dict[smoothing_scheme]
    print(irs_mpc.irs_mpc_params.bundle_mode)

    q_u_goal = np.array(
        [q_u_goal_dict["y"], q_u_goal_dict["z"], q_u_goal_dict["theta"]]
    )
    x0, u0, q_u0 = get_x0_and_u0_from_json(q_u0_json, q_a0_json)
    x_goal = np.array(x0)

...
def compute_min_metric(num_nodes: int, q_query):
    ...
    dist = irs_rrt.calc_distance_batch_local(
        ...
        num_nodes, is_q_u_only=True)
    return np.min(dist), np.argmin(dist)

def create_tree_plot_up_to_node(num_nodes: int):
    nodes_plot = plt.plot(q_u_nodes[:num_nodes, 0],
                             q_u_nodes[:num_nodes, 1], 'ro')

    edges_plot = plt.plot(z_edges[:(num_nodes - 1) * 3],
                          theta_edges[:(num_nodes - 1) * 3],
                          'k')
    ...
    for i in range(num_nodes):
        ...
        ellipse = patches.Ellipse(node.q[-2:], major, minor, theta,
            facecolor=(0,1,0,0.1))
    ...
    for i in range(100):
        for j in range(100):
            zv[i,j], zv_ind[i,j] = compute_min_metric(num_nodes,
                np.array([xv[i,j], yv[i,j]]))
    ...
    for i in range(100):
        for j in range(100):
            color = colormap(
                np.argwhere(
                    unique_coloring == zv_ind[i,j]) / len(unique_coloring)
            )[0][0]

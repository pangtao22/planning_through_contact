...
q_vis.draw_object_triad(length=0.1, radius=0.01, opacity=1, path="door/sphere")
...
std_u_initial[0:3] = 0.05
impc_params.std_u_initial = std_u_initial
...
for i_s, (t_start, t_end) in enumerate(sub_segments):
    ...
    n_steps_per_h = max(10, int(np.ceil(10 / len(u_trj))))
...
# %%
with open("door_optimized_q_and_u_trj.pkl", "wb") as f:
    pickle.dump(
        {
